<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="InstrumrntManagement.aspx.cs" Inherits="Hospital.Views.SystemManagement.InstrumentManagement.InstrumrntManagement" %>
               ...
               <form runat="server">
                <div class="content-wrapper">
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">                       
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">检查仪器表</h4>
                                <p class="card-description">
                                   Instrument Information Table
                                </p>
                                <label>仪器编号</label>
                                <input id="iidtext"  runat="server"/>
                                 <label>名称</label>
                                <input id="inametext"  runat="server"/>
                                                        ...
                                                        <table class="table">
                                                            ...
                                                            <tbody>
                                                                <%for (i = 0; i < instruments.Count; i++)
                                                                    {%>
                                                                <tr>
                                                                    <td><p><a href="UpdateInstrument.aspx?I_ID=<%=Convert.ToString(instruments[i].I_ID) %>"><%=Convert.ToString(instruments[i].I_ID) %></a></p></td>
                                                                    <td><p><a href="UpdateInstrument.aspx?I_Name=<%=instruments[i].I_Name %>"><%=instruments[i].I_Name %></a></p></td>
                                                                    <td><p><a href="UpdateInstrument.aspx?I_Number=<%=Convert.ToString(instruments[i].I_Number)%>"><%=Convert.ToString(instruments[i].I_Number)%></a></p></td>
                                                                    <td><p><a href="UpdateInstrument.aspx?DE_ID=<%=Convert.ToString(instruments[i].DE_ID) %>"><%=Convert.ToString(instruments[i].DE_ID) %></a></p></td>
                                                                </tr>
                                                                <%} %>
                                                            </tbody>
                                                        </table>
